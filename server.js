...
function calculateScore(selectedWord, allPlayers) {
  let count = 0;
  const totalPlayers = allPlayers.length;
  allPlayers.forEach(player => {
    if (player.words && player.words.includes(selectedWord)) count++;
  });
  if (count === 0) return 0;
  if (count === 1) return 0;
  if (count === 2) return 4; // 2명은 전원이어도 4점
  if (count === totalPlayers) return 0; // 3명 이상 전원이면 0점
  return count;
}
...
function endRound(room, roomCode) {
  ...
  const scores = room.players.map(p => ({
    ...
    roundScore: room.roundScores[p.id] || 0, totalScore: p.score
  }));
  ...
  io.to(roomCode).emit('roundEnd', { scores, winners, losers, roundScores: room.roundScores });
}

function startNextRound(room, roomCode) {
  const lastPlaceIds = room.lastLastPlace || [];
  room.players.forEach(p => { p.words = []; p.usedWords = []; p.wordsSubmitted = false; });
  if (lastPlaceIds.length > 0) {
    const lastPlacePlayers = room.players.filter(p => lastPlaceIds.includes(p.id));
    const otherPlayers = room.players.filter(p => !lastPlaceIds.includes(p.id)).sort((a, b) => a.joinOrder - b.joinOrder);
...
io.on('connection', (socket) => {
  ...
  socket.on('createRoom', ({ nickname, title, password }) => {
    ...
    rooms[roomCode] = {
      code: roomCode, title: title || '공감 게임',
      password: password || '', host: socket.id,
      lastActivity: Date.now(),
      players: [{ id: socket.id, nickname, ready: false, score: 0, words: [], usedWords: [], joinOrder: 0 }],
      status: 'waiting', gamePhase: 'topic', topic: '',
      currentPlayerIndex: 0, turnCount: 0, roundScores: {}, lastLastPlace: null,
    };
  ...
  socket.on('joinRoom', ({ nickname, roomCode, password }) => {
    ...
    room.players.push({ id: socket.id, nickname, ready: false, score: 0, words: [], usedWords: [], joinOrder: room.players.length });
  ...
  socket.on('startGame', () => {
    ...
    room.turnCount = 0;
    room.roundScores = {};
    room.players.forEach(p => { p.score = 0; p.words = []; p.usedWords = []; room.roundScores[p.id] = 0; });
  ...
  socket.on('submitWords', ({ words }) => {
    ...
    const player = room.players.find(p => p.id === socket.id);
    if (!player) return;
    const cleanWords = words.map(w => w.replace(/[^가-힣a-zA-Z0-9]/g, '').slice(0, 10)).filter(w => w.length > 0).slice(0, 10);
    if (cleanWords.length !== 10) return socket.emit('wordsError', '단어를 10개 모두 입력해주세요.');
    if (new Set(cleanWords).size !== 10) return socket.emit('wordsError', '중복된 단어가 있어요. 10개 모두 다르게 입력해주세요.');
    ...
    if (allSubmitted) {
      ...
      room.currentPlayerIndex = 0;
      room.turnCount = 0;
      io.to(socket.roomCode).emit('selectionPhaseStart', { room: sanitizeRoom(room), currentPlayerId: room.players[0].id });
    } else {
  ...
  socket.on('selectWord', ({ word }) => {
    ...
    room.lastActivity = Date.now();
    const score = calculateScore(word, room.players);
    room.roundScores[socket.id] = (room.roundScores[socket.id] || 0) + score;
    const matchPlayers = room.players.filter(p => p.words.includes(word)).map(p => p.nickname);
    io.to(socket.roomCode).emit('wordSelected', {
      playerId: socket.id, playerNickname: currentPlayer.nickname,
      word, score, matchPlayers, roundScores: room.roundScores
    });
    room.turnCount++;
    ...
    if (room.turnCount >= totalTurns) {
      ...
      io.to(socket.roomCode).emit('nextTurn', { currentPlayerId: nextPlayer.id, turnCount: room.turnCount, totalTurns });
    }
  });
  ...
  socket.on('disconnect', () => {
    ...
    io.to(socket.roomCode).emit('playerLeft', { room: sanitizeRoom(room), leftNickname: socket.nickname });
  });
});
